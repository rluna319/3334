...
While phase 1 got the commands up and running. Phase 2 implements pass 1 of 
the 2 pass assembler. Pass 1 will use the assemble command to generate a symbol 
table and an intermediate file. Phase 3 implements Pass 2 of the 2 pass
assembler. Pass 2 will create the object file and listing file from the newy 
generated intermediate file and symbol table. Phase 4 now implements the rest
of the commands available to the SIC simulator. These commands are LOAD, DUMP,
EXECUTE, and DEBUG.  The following are the names of the files being generated... 
        
        Symbol Table: 'SymbolTable.txt' 
        Intermediate File: 'Intermediate.txt'
        Listing File: '*source file name*_obj.txt'
        Object File: '*source file name*_list.txt'  <-- fails 

Comment 1: Pass 2 of the assembler is broken. I did my best attempt at it but it
just prints out garbage to the listing file and doesn't reach the END directive.
Without reaching the END directive Pass 2 see's this as an error and is 
programmed to delete the object file that is being created. Reason being, is that
it was supposed to create the object file only under the conditions that there 
are no errors in the source code.

Comment 2: DEBUG is an available and working command!  
...
void loadf(char *prm1)
{
	...
	while(fgets(LINE, 128, obj)){
		...
		else if (LINE[0] != 'T' || strlen(LINE) < 19) {
	...
	if (!Err) printf("Load complete!\n");
	else printf("Load incomplete...\n");
